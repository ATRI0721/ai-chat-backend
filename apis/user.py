...
from curd import add_user, get_user_by_email, update_user_password
import curd
from emails.utils import verify_email
from models.database import User
from models.interfaces import UserCreate, UserLoginCode, UserResponse, UserLoginResponse, UserLoginPassword, UserResetPassword
...
def register(user: UserCreate, session: SessionDep):
    if not verify_email(user.email, user.verification_code):
...
def delete_user(user:CurrentUser,session: SessionDep):
    curd.delete_user(user.id,session)
